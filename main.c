...
#define YOUR_APPLICATION_ID      "139475"     // SET YOUR APPLICATION ID
#define YOUR_APPLICATION_NAME    "Mediaschlingel" // SET YOUR APPLICATION NAME
...
static void app_launch_pause();

static void app_launch_resume();
...
int main(int argc, char *argv[]) {
    ...
    if (argc != 3) {
        log("invalid amout of parameter.\n")
            return -1;
    }

    if (strlen(argv[2]) == 0) {
        log("dz_track value must be set to a value.\n");
        return -1;
    }

    strcpy(sz_track_url, argv[2]);
    log("dezzer Track %s\n", sz_track_url);
...
static void app_launch_pause(){
    log("PAUSE CURRENT SONG. => %s\n", sz_track_url);
    dz_player_pause(dzplayer, NULL, NULL);
}

static void app_launch_resume(){
    log("RESUME CURRENT SONG. => %s\n", sz_track_url);
    dz_player_resume(dzplayer, NULL, NULL);
}
